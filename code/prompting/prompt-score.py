...
def find_few_shots(pool_data_path, few_shot_type, source_sent, target_sent, target_error_type, target_error_severity, num_few_shots):
    ...
    for data in total_dataset:
        if data["score"] != "None" and data["error_type"].lower() == target_error_type and data["severity"].lower() == target_error_severity and data["original_src"] != source_sent and data["original_tgt"] != target_sent:
...
# Score
def score_prompt_template(source_lang, target_lang, error_type, error_severity, source_sent, target_sent, error_score):
    pattern = r"<v>(.*?)</v>"
    match = re.search(pattern, target_sent)
    if match:
        error_span = match.group(1)
        error_span = error_span.strip()
        prompt = f"(1) There is an error scored as {error_score} in 0 to 100 range.\n{source_lang}: {source_sent}\n{target_lang}: {target_sent}\nImproved {target_lang}:"
    else:
        prompt = f"(1) There is an error scored as {error_score} in 0 to 100 range.\n{source_lang}: {source_sent}\n{target_lang}: {target_sent}\nImproved {target_lang}:"
    return prompt

def score_fewshot_prompt_template(source_lang, target_lang, error_type, error_severity, source_sent, target_sent, gold_sent, error_score):
    pattern = r"<v>(.*?)</v>"
    match = re.search(pattern, target_sent)
    if match:
        error_span = match.group(1)
...
def main():
    ...
    # =========================================== Load Dataset ===========================================
    score_instruction = f"You are a professional post-editor. Improve the translation from {args.source_lang} to {args.target_lang} based on the identified errors without any explanation."
    ...
    with jsonlines.open(args.output_path, mode="w") as outfile:
        with jsonlines.open(args.input_path) as file:
            for line in file.iter():
                ...
                error_score = line["score"]

                if error_score != "None":
    # ====================================== Few shot Construction ========================================
                    prompts.append(score_instruction)
                    if args.num_few_shots > 0:
                        ...
                        for shot in few_shots:
                            prompts.append(score_prompt_template(args.source_lang, args.target_lang, error_type, error_severity, shot["original_src"], shot["original_tgt"], shot["gold_tgt"], shot["score"]))
                    query_prompt = score_fewshot_prompt_template(args.source_lang, args.target_lang, error_type, error_severity, source_sent, target_sent, error_score)
