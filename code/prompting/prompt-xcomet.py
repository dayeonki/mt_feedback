...
def find_few_shots(args, pool_data_path, few_shot_type, source_sent, target_sent, target_error_severity, error_span, num_few_shots):
    total_dataset = []
    ...
    for data in total_dataset:
        for severity in data["severity"]:
            if severity.lower() in target_error_severity and data["original_src"] != source_sent and data["original_tgt"] != target_sent:
    ...
    for shot in few_shots:
        few_shot_prompts.append(xcomet_fewshot_prompt_template(args.source_lang, args.target_lang, shot["severity"], shot["error_span"], shot["original_src"], shot["original_tgt"], shot["gold_tgt"]))
    return few_shot_prompts


# xCOMET
def xcomet_prompt_template(source_lang, target_lang, error_severity, error_span, source_sent, target_sent):
    all_listings = []
    for i in range(len(error_severity)):
        listing = f"({i+1}) There is a {error_severity[i]} error at \"{error_span[i]}\"."
        all_listings.append(listing)
    single_listing = "\n".join(all_listings)
    prompt = f"{single_listing}\n{source_lang}: {source_sent}\n{target_lang}: {target_sent}\nImproved {target_lang}:"
    return prompt

def xcomet_fewshot_prompt_template(source_lang, target_lang, error_severity, error_span, source_sent, target_sent, gold_sent):
    all_listings = []
    for i in range(len(error_severity)):
        listing = f"({i+1}) There is a {error_severity[i]} error at \"{error_span[i]}\"."
        all_listings.append(listing)
...
def main():
    ...
    # =========================================== Load Dataset ===========================================
    xcomet_instruction = f"Improve the translation from {args.source_lang} to {args.target_lang} based on the identified errors without any explanation."
    ...
    # ====================================== Few shot Construction ========================================
                prompts.append(xcomet_instruction)
                if args.num_few_shots > 0:
                    few_shots = find_few_shots(args, args.pool_data_path, args.few_shot_type, source_sent, target_sent, error_severity, error_span, args.num_few_shots)
                    for shot in few_shots:
                        prompts.append(shot)
                query_prompt = xcomet_prompt_template(args.source_lang, args.target_lang, error_severity, error_span, source_sent, target_sent)
